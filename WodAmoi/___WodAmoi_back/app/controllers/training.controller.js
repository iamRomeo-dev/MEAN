const db = require("../models_training");
...
exports.create = (req, res) => {
  ...
  // Crée un entrainement
  const entraineMoi = new Entrainement({
    title: req.body.title,
    date: req.body.date,
    equipe: req.body.equipe,
    mouvements: req.body.mouvements,
    description: req.body.description
  });

  // Save entrainement in the database
  entraineMoi
    .save(entraineMoi)
    .then(data => {
      res.send(data);
...
exports.findAllPublished = (req, res) => {
  Entrainement.find({ published: true })
    .then(data => {
      res.send(data);
    })
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving entrainements."
      });
    });
};
